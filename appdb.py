...
class User(Base):
	...
	uuid = Column(String(128), unique=True) # Valid chars: ABCDEFGHJKLMNPQRSTUWXYZ23456789
	login = Column(String(128)) # Login name, can be used in URLs
	name = Column(String(128)) # Name/nick the user wants to be called and displayed as
	legal_name = Column(String(256)) # Legal name as in governmental records
	residence = Column(String(512)) # Users's place of residence. City/town/municipality + country
	city = Column(String(256)) # User's official home city/town/municipality
	country = Column(String(256)) # User's official home country
	address_id = Column(Integer, ForeignKey('address.id', onupdate="RESTRICT", ondelete="RESTRICT")) # Current address of user from Address table
	...
	dob = Column(Date) # User's official date of birth
	ssn = Column(String(256)) # Official social security number of the user
	location = Column(String(256)) # Users gps coordinates
	joined = Column(DateTime) # Time when user initially joined
	last_seen = Column(DateTime) # Time when user was last seen active on system
	@staticmethod
...
class Address(Base):
	...
	user_id = Column(Integer, ForeignKey('user.id', onupdate="RESTRICT", ondelete="RESTRICT", use_alter=True, name="user_id_altc")) # Reference to the id of the user having this address
	line1 = Column(String(512)) # First address line; name of the recipient
	line2 = Column(String(512)) # Second address line; company, etc
	street = Column(String(512)) # Street address
	zipcode = Column(String(128)) # ZIP number in international format
	city = Column(String(512)) # City/town/municipality of the recipient
	state = Column(String(512)) #  State, if applicable
	country = Column(String(512)) # Country

		   
class Auth(Base):
	__tablename__ = 'auth'
	id = Column(Integer, primary_key=True) # Id of this token
	user_id = Column(Integer, ForeignKey('user.id')) # Reference to the id of the user having this token
...
class Organization(Base):
	...
	legal_name = Column(String(512)) # Full legal name of the organization
...
class Membership(Base):
	...
	user_id = Column(Integer, ForeignKey('user.id', onupdate="RESTRICT", ondelete="RESTRICT")) # User id this membership applies to
	organization_id = Column(Integer, ForeignKey('organization.id', onupdate="RESTRICT", ondelete="RESTRICT")) # Organization this membership applies to
	status = Column(Enum('applied', 'member', 'honorary_member', 'expelled', 'resigned' , name='status_types'), nullable=False) # Status type
	position = Column(Enum('nonpriv', 'chair', 'vice_chair', 'secretary', 'board' , name='position_types'), nullable=False) # Position type
	title = Column(String(256)) # Description of a special position in the organization
	application = Column(String(512)) # Includes UUID string and legal name of the applicant
...
class MetaDef(Base):
	...
	organization_id = Column(Integer, ForeignKey('organization.id', onupdate="RESTRICT", ondelete="RESTRICT")) # Organization this metadata applies to.
	label = Column(String(256)) # Label of the metadata
	type = Column(Enum('string', 'checkbox', 'date', name='meta_types'), nullable=False) # Metadata type
	readonly = Column(Boolean) # Is the metadata editable by the user
...
class Log(Base):
	...
	time = Column(DateTime) # Time of this log entry
	ip = Column(String(45)) # IP address of the request generating this log entry
	logged_user_id = Column(Integer, ForeignKey('user.id', onupdate="RESTRICT", ondelete="RESTRICT")) # User id of the user performing the action
